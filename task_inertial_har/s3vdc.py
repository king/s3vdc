...
from lib.resolve_4d_input_blob import resolve_4d_input_blob
...
def get_cvae(
    ...
    # z_mu /= tf.sqrt(tf.reduce_sum(tf.square(z_mu)))
    code.append(z_mu)
    ...
    code.append(z)
    result = tf.cond(
        global_step <= (hparams.gammaSteps + math.ceil(hparams.gmmSteps)),
        lambda: z_mu,
        lambda: z,
    )
    ...
    for i in range(n_filters - 2, -2, -1):
        _layer = tf.layers.Conv2DTranspose(
            ...
            activation=None if i - 1 == -2 else resolve_activation_fn(hparams),
            trainable=True,
        )
        result = _layer.apply(result)
...
class S3VDCHook(tfe.SessionRunHook):
    ...
    def __init__(
        ...
        self.gamma_gmm_training_steps = self.gamma_steps + math.ceil(self.gmm_steps)
        ...
        # self.mtm = tf.pow(1. - self.vgmm['anneal_factor'], 3)
        self.update_svdc_with_gmm_ops = [
            tf.assign(
                self.vgmm["sigma"],
                self.sigma
                # (1. - self.mtm) * self.vgmm['sigma'] + self.mtm * self.sigma
            )
        ]
        self.update_svdc_with_kmeans_ops = [
            tf.assign(
                self.vgmm["mu"],
                self.mu
                # (1. - self.mtm) * self.vgmm['mu'] + self.mtm * self.mu
            )
        ]
    ...
    def skl_init(self, session: tf.Session, step: int) -> None:
        ...
        # make less than 256 possible
        if self.gmm_steps < 1:
            n_subsample = int(self.gmm_steps * 256)
            if n_subsample < 1:
                n_subsample = 1
            gmm_input_slice = gmm_input_slice[:n_subsample, :]
        # append to the current cache
        ...
        if is_final:
            ...
            self.gmm_data = None
            tfl.info("GMM init done at local step {}".format(step))
    ...
    def before_run(self, run_context: tfe.SessionRunContext) -> None:
        ...
        if gs_offset <= 0:
            # vae training
            if gs_offset <= -math.ceil(self.gmm_steps):
...
def estimator_fn(
    ...
    utils["batch_size"] = tf.shape(
        tfc.input_layer(only_features, only_feature_columns)
    )[0]
    input_feature = resolve_4d_input_blob(
        hparams=hparams,
        runtime_bs=utils["batch_size"],
        features=only_features,
        feature_columns=only_feature_columns,
        info_log=True if mode == tfe.ModeKeys.TRAIN else False,
    )
    ...
    if mode == tfe.ModeKeys.TRAIN:
        input_feature_noise += tf.random_normal(
            shape=tf.shape(input_feature), mean=0.0, stddev=1e-12, dtype=tf.float32
        )
    ...
    with tf.device("/device:CPU:0"):
        with tf.variable_scope("VGMM"):
            ...
            vgmm["latent_loss"] = tf.reduce_mean(vgmm["latent_loss"]) * 0.02

            # S3VDC reconstruction loss
            if resolve_simple_bool(hparams, "useSigmoidReconstructionLoss"):
                vae_out = vae["output"]
                vae_out = tf.div(
                    tf.subtract(vae_out, tf.reduce_min(vae_out)),
                    tf.subtract(tf.reduce_max(vae_out), tf.reduce_min(vae_out)) + 1e-12,
                ) * (50.0)
                vgmm["reconstruction_loss"] = tf.reduce_mean(
                    tf.losses.sigmoid_cross_entropy(
                        multi_class_labels=input_feature, logits=vae_out
                    )
                )
            else:
    ...
    if mode != tfe.ModeKeys.PREDICT:
        ...
        if mode == tfe.ModeKeys.TRAIN:
            ...
            post_training_step = tf.clip_by_value(
                utils["global_step"] - hparams.gammaSteps - math.ceil(hparams.gmmSteps),
                0,
                hparams.maxSteps,
            )
