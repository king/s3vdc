...
_this_root = os.path.join(os.path.dirname(task_fashion.__file__), "../")
shutil.copyfile(
    os.path.join(_this_root, "task_fashion/s3vdc.py"),
...
conf_parser = TrainDevConfigParser(
    data_utils.get_file_contents(
        os.path.join(os.path.dirname(task_fashion.__file__), "config.json")
    )
)
